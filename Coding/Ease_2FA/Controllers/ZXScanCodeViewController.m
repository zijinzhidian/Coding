...
- (void)configUI{
    ...
    CGRect scanRect = CGRectMake(padding, kDevice_Is_iPhoneX ? kScreen_Height/10 + 84 : kScreen_Height/10, width, width);
    ...
    if (!_videoPreviewLayer) {
        ...
        if (!input) {
            [UIAlertController showAlertViewWithTitle:error.localizedDescription];
            ...
            if (![captureMetadataOutput.availableMetadataObjectTypes containsObject:AVMetadataObjectTypeQRCode]) {
                [UIAlertController showAlertViewWithTitle:@"摄像头不支持扫描二维码！"];
                [self.navigationController popViewControllerAnimated:YES];
            }else{

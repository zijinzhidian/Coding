...
@interface OTPListViewController ()<UITableViewDelegate,UITableViewDataSource>
...
- (void)beginButtonClicked:(id)sender {
    ...
    ZXScanCodeViewController *vc = [[ZXScanCodeViewController alloc] init];
    ...
    if (self.authURLs.count > 0) {
        ...
        if (!_myTableView) {
            _myTableView = ({
                ...
                [tableView registerClass:[TOTPTableViewCell class] forCellReuseIdentifier:NSStringFromClass([TOTPTableViewCell class])];
                [tableView registerClass:[HOTPTableViewCell class] forCellReuseIdentifier:NSStringFromClass([HOTPTableViewCell class])];
                [tableView mas_makeConstraints:^(MASConstraintMaker *make) {
                    make.edges.equalTo(self.view);
                }];
                tableView;
            });
            [self.navigationItem setRightBarButtonItem:[[UIBarButtonItem alloc] initWithImage:[UIImage imageNamed:@"addBtn_Nav"] style:UIBarButtonItemStylePlain target:self action:@selector(beginButtonClicked:)]];
        }
...
+ (NSMutableArray *)loadKeychainAuthURLS {
    //获取所有帐号信息
    NSArray *otpAccountList = [SAMKeychain accountsForService:kOTPService];
    ...
    for (NSDictionary *otpAccount in otpAccountList) {
        //获取帐号密码数据
        NSData *passwordData = [SAMKeychain passwordDataForService:kOTPService account:otpAccount[(__bridge id)kSecAttrAccount]];
        if (passwordData) {
...
- (void)dealWithScanResult:(NSString *)resultStr ofVC:(ZXScanCodeViewController *)vc {
    ...
    if ([authURL isKindOfClass:[TOTPAuthURL class]]) {
        ...
        [UIAlertController showAlertViewWithTitle:tipStr cancelButtoonTitle:nil sureButtonTitle:@"重试" sureHandler:^{
            if (![vc isScaning]) {
                [vc startScan];
            }
        }];
...
- (void)addOneAuthURL:(OTPAuthURL *)authURL {
    ...
    for (OTPAuthURL *item in self.authURLs) {
        if ([authURL.name isEqualToString:item.name]) {
            ...
            if ([authURL.otpCode isEqualToString:item.otpCode]) {
                [UIAlertController showAlertViewWithTitle:[NSString stringWithFormat:@"该二维码已被保存为账户名：\n%@", authURL.name]];
            } else {
                [UIAlertController showAlertViewWithTitle:@"提示" message:[NSString stringWithFormat:@"账户名：%@ 已存在\n选择 '更新' 覆盖原账户。", authURL.name] cancelButtoonTitle:@"取消" sureButtonTitle:@"更新" sureHandler:^{
                    //保存
                    if ([authURL saveToKeychain]) {
                        ...
                        [UIAlertController showAlertViewWithTitle:@"保存过程中发生了异常，请重新扫描"];
                    }
                }];
            }
        }
    ...
    if (!alreadyHave) {
        if ([authURL saveToKeychain]) {
            ...
            [self configUI];
            [UIAlertController showAlertViewWithTitle:[NSString stringWithFormat:@"添加账户成功：\n%@", authURL.name]];
        } else {
            [UIAlertController showAlertViewWithTitle:@"保存过程中发生了异常，请重新扫描"];
        }
    }
...
+ (NSString *)otpCodeWithGK:(NSString *)global_key {
    ...
    if (global_key.length > 0) {
        ...
        for (OTPAuthURL *authURL in authURLs) {
            ...
            //服务端和客户端验证
            if ([cur_issure isEqualToString:@"Coding"] && [cur_global_key isEqualToString:global_key]) {
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if ([authURL isKindOfClass:[TOTPAuthURL class]]) {
        cellIdentifier = NSStringFromClass([TOTPTableViewCell class]);
    } else {
        cellIdentifier = NSStringFromClass([HOTPTableViewCell class]);
    }
...
- (void)tableView:(UITableView *)tableView commitEditingStyle:(UITableViewCellEditingStyle)editingStyle forRowAtIndexPath:(NSIndexPath *)indexPath {
    if (editingStyle == UITableViewCellEditingStyleDelete) {
        OTPAuthURL *authURL = self.authURLs[indexPath.section];
        __weak typeof(self) weakSelf = self;
        [UIAlertController showAlertViewWithTitle:@"删除此账户不会停用两步验证" message:@"\n您可能会因此无法登录自己的账户\n在删除该账户前，请先停用两步验证，或者确保您可以通过其它方法生成验证码。" cancelButtoonTitle:@"取消" sureButtonTitle:@"确认删除" sureHandler:^{
